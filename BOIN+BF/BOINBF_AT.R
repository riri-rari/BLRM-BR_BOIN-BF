...
#### backfill_arm()
backfill_arm <- function(cohort, doses_info, new_reference, run, pts, previous_time, limit_time, n_max, i_simulation, current_dose){
    ...
    for (back_pts in temp_pts){
      ...
      print(c('Patient put at', current_backfill_dose))
  ...
  return(list('cohort' = cohort, 'doses_info' = doses_info, 'discarded' = discarded, 'pts' = pts, 'last_arrival' = last_arrival, 'last_lag' = last_lag))
}
...
compute_BFBOIN_at <- function(cohort, doses_info, new_reference, cohortsize, n_max, n_cap, n_stop, DLT_time, lambda, i_simulation = 0){
  ...
  while (pts <= n_max){
    ...
    #has my pts a DLT? so then classic way of decision 
    patient <- cohort %>% filter(Run == run) %>% head(1) %>% select(dlt = DLT)
    
    if(patient$dlt == 0 && current_cohortsize == 1){
      print(c('no DLT', patient$dlt))
      
      #no DLT observed so we keep on with escalation. The possiblity for backfill is open 
    ...
    if(backfill == T){
      ...
      n_resp <- 0
    for (i in (pts-current_cohortsize):(pts - 1) ){
    ...
    backfill <- backfill_arm(cohort = cohort, doses_info = doses_info, new_reference = new_reference, run = run, pts = pts, previous_time = previous_time, limit_time = limit_time, n_max , i_simulation = i_simulation, current_dose = current_dose)
     
    cohort <- transform_data(backfill$cohort)
    doses_info <- transform_data_doses(backfill$doses_info)
    pts <- backfill$pts
    last_arrival <- backfill$last_arrival
    last_lag <- backfill$last_lag
    discarded <- backfill$discarded
    ...
    if(is.na(next_dose$safe_dose)){
      ...
      #1-print(c('actual dose', current_dose))
        total_cohort_pts <- cohort %>% filter(Dose == current_dose) %>% summarise(pts = n())
        #1-print(c('total cohort', total_cohort_pts$pts))
        if(total_cohort_pts$pts >= n_stop){
          #print('maximum number + stay')
          n_stop_reached <- n_stop_reached + 1
          break 
        }
    } else if (next_dose$change == 3){
      if (current_dose == min(doses)){
        #1-print(c('actual dose min', current_dose))
        total_cohort_pts <- cohort %>% filter(Dose == current_dose) %>% summarise(pts = n())
        #1-print(c('total cohort min', total_cohort_pts$pts))
        if(total_cohort_pts$pts >= n_stop){
          #print('maximum number + stay')
          n_stop_reached <- n_stop_reached + 1
          break 
        ...
        #close upper 
        state <- ifelse(doses_info$Dose >= current_dose, 0, doses_info$State)
         #1-print(c('close upper with states', state))
        doses_info$State <- state
        }
      }
      ...
      if(current_dose == max(doses_info$Dose)){
        #1-print(c('actual dose max', current_dose))
        total_cohort_pts <- cohort %>% filter(Dose == current_dose) %>% summarise(pts = n())
        #1-print(c('total cohort min', total_cohort_pts$pts))
        if(total_cohort_pts$pts >= n_stop){
          #print('maximum number + stay')
  ...
  #check for opening of previous not-responsive  (since then i filter for the values of backfill dose < current_dose, i am protected in any case ) only if the next choice is not to eliminate the current dose (ok for descalating as we are just looking at the possibility for backfill)
       ...
       #check cap for the backfill (if you can open but it has limit pts then put at 2)
        pts_check <- cohort %>% filter(Dose == current_dose) %>% summarise(pts = n())
         if (n_resp >= 1){
            ...
            doses_info[index, ]$State <- ifelse(pts_check$pts < n_cap, 1, 2)
            #open also those that are larger than this dose. in any case I am protected since I have < cohort_dose
            doses_info[(index + 1):nrow(doses_info), ]$State <- ifelse(doses_info[(index + 1):nrow(doses_info), ]$State == -1, 1,doses_info[(index + 1):nrow(doses_info), ]$State )
            
          }
      #print(c('chnaged', doses_info$State))
      #print(run)
    }    
...
simulate_BFBOIN_at <- function(n_simulation, target = 0.3, real_mtd){
